...
// Global variable to persist the page cache in server-side Node.js memory
const globalForPageCache = globalThis as unknown as {
  cachedPageData: CachedPageData | null;
};

if (!globalForPageCache.cachedPageData) {
  globalForPageCache.cachedPageData = null;
}

const PAGE_CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes

function getPageCache(): CachedPageData | null {
  const cache = globalForPageCache.cachedPageData;
  if (!cache) return null;
  const now = Date.now();
  if (now - cache.timestamp < PAGE_CACHE_TTL_MS) {
    console.log('[PageCache] Serving homepage from cache. Age:', now - cache.timestamp, 'ms');
    return cache;
  }
  return null;
}

function setPageCache(indicesResult: IndicesResult, topArticle: Article, gridArticles: Article[]) {
  console.log('[PageCache] Saving homepage data to cache');
  globalForPageCache.cachedPageData = {
    indicesResult,
    topArticle,
    gridArticles,
    timestamp: Date.now(),
...
export default async function HomePage() {
  ...
  if (cached) {
    const { indicesResult, topArticle, gridArticles } = cached;
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <section className="mb-10 text-center">
            ...
            <div className="flex items-center justify-center gap-3 text-sm text-gray-500 dark:text-gray-400">
              ...
              <span>•</span>
              <time dateTime={topArticle.publishedAt}>
                {new Date(topArticle.publishedAt).toLocaleString("en-IN", {
                  day: "numeric",
                  month: "short",
                  hour: "2-digit",
                  minute: "2-digit",
  ...
  // Save resolved page data to global cache
  setPageCache(indicesResult, topArticle, gridArticles);
